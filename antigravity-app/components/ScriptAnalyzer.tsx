...
import SynastryOverlay from '@/components/SynastryOverlay';
import NatalChart2D from '@/components/NatalChart2D';
...
export default function ScriptAnalyzer() {
    ...
    function saveUserToLibrary(data: BirthData) {
        ...
        // We use 'city' (Name) as ID for simplicity in this context, or we could add an ID field.
        // Let's assume Name + Date is unique enough, or just Name.
        // For better UX, let's update if name matches, else add.

        setUserLibrary(prev => {
            const existingIndex = prev.findIndex(u => u.city === data.city);
            let updated;
            if (existingIndex >= 0) {
                updated = [...prev];
    ...
    function deleteUserFromLibrary(name: string) {
        ...
        setUserLibrary(prev => {
            const updated = prev.filter(u => u.city !== name);
            localStorage.setItem('userLibrary', JSON.stringify(updated));
            return updated;
        });
    }
    ...
    const PlanetaryTrinity = ({ sun, moon, asc }: { sun: string, moon: string, asc: string }) => (
        <div className="grid grid-cols-3 gap-2 mt-4">
            <div className="bg-white border border-black/5 p-3 rounded-lg flex flex-col items-center text-center">
                <div className="text-[#C55959] text-xl mb-1">☉</div>
                <div className="text-[9px] uppercase font-bold tracking-widest text-gray-400 mb-1">Sol</div>
                <div className="font-serif font-bold text-sm text-[#1a1a1a]">{sun || '-'}</div>
                <p className="text-[8px] text-gray-500 mt-1 leading-tight">Esencia Vital & Voluntad</p>
            </div>
            <div className="bg-white border border-black/5 p-3 rounded-lg flex flex-col items-center text-center">
                <div className="text-[#C55959] text-xl mb-1">☾</div>
                <div className="text-[9px] uppercase font-bold tracking-widest text-gray-400 mb-1">Luna</div>
                <div className="font-serif font-bold text-sm text-[#1a1a1a]">{moon || '-'}</div>
                <p className="text-[8px] text-gray-500 mt-1 leading-tight">Mundo Interior & Emoción</p>
            </div>
            <div className="bg-white border border-black/5 p-3 rounded-lg flex flex-col items-center text-center">
                <div className="text-[#C55959] text-xl mb-1">↑</div>
                <div className="text-[9px] uppercase font-bold tracking-widest text-gray-400 mb-1">Ascendente</div>
                <div className="font-serif font-bold text-sm text-[#1a1a1a]">{asc || '-'}</div>
                <p className="text-[8px] text-gray-500 mt-1 leading-tight">Máscara & Camino</p>
            </div>
        </div>
    );
                            ...
                            <div className="glass-panel p-8 relative overflow-hidden group h-fit">
                                ...
                                <div className="flex flex-col gap-6 mb-8 relative z-10">
                                    ...
                                    <div className="group/input relative">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                onChange={e => setCurrentUser(prev => prev ? { ...prev, city: e.target.value } : { city: e.target.value, date: new Date().toISOString(), latitude: 40, longitude: -3 })}
                                            />
                                            ...
                                            <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-black/10 shadow-2xl rounded-lg z-50 max-h-64 overflow-y-auto animate-fadeIn">
                                                ...
                                                {userLibrary.map((user, i) => (
                                                    <div key={i} className="p-3 border-b border-black/5 hover:bg-[#F9F8F4] group flex justify-between items-center transition-colors">
                                                        ...
                                                        >
                                                            <div className="font-bold text-sm text-[#1a1a1a]">{user.city}</div>
                                    ...
                                    <button onClick={() => currentUser && handleSaveUser(currentUser)} className="btn-primary self-start bg-[#1a1a1a] hover:bg-[#C55959] text-[#F9F8F4] border border-white/10 px-8 py-3 w-full flex items-center justify-center gap-2 group transition-all mt-2 shadow-lg">
                                        ...
                                        <span className="uppercase tracking-widest text-xs font-bold">{userLibrary.some(u => u.city === currentUser?.city) ? 'Actualizar' : 'Cristalizar'}</span>
                                    </button>
                                </div>
                            </div>

                            {/* TRANSITS PANEL (Moved to Sidebar) */}
                                        ...
                                        <div key={i} className="flex items-start gap-2 p-2 bg-white/60 rounded border border-black/5">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-1 mb-px">
                                                    <span className="font-bold text-[10px] uppercase">{aspect.planet1}</span>
                                                    <span className="text-[8px] text-gray-400">vs</span>
                                                    <span className="font-bold text-[10px] uppercase">{aspect.planet2}</span>
                                                </div>
                                                <p className="text-[10px] font-serif italic text-gray-600 leading-tight">{aspect.descriptionEs}</p>
                                            </div>
                                        </div>
                                    )) : <div className="text-center py-8 text-gray-400 italic text-xs">Sin tránsitos mayores.</div>}
                                </div>
                            </div>
                        </div>

                        {/* RIGHT COLUMN: CHART (8 Cols) */}
                        <div className="lg:col-span-8 flex flex-col gap-6 h-full">
                            <div className="glass-panel p-8 flex items-center justify-center min-h-[600px]">
                                <div className="w-full max-w-2xl relative z-0">
                                    <ChartZoomWrapper title={`Carta Natal: ${currentUser?.city || 'Usuario'}`}>
                                        {currentUser ?
                                            <NatalChart2D date={currentUser.date} latitude={currentUser.latitude} longitude={currentUser.longitude} transitsDate={transitDate.toISOString()} /> :
                                            <div className="h-full flex items-center justify-center opacity-30 font-serif italic">"El conocimiento es la puerta..."</div>
                                        }
                                    </ChartZoomWrapper>
                                </div>
                            </div>
                            {userSigns && <PlanetaryTrinity sun={userSigns.sun} moon={userSigns.moon} asc={userSigns.ascendant} />}
                        </div>
                    </div>
                )}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                                                    ...
                                                    <div className="md:col-span-8 flex flex-col gap-6 min-h-[600px]">
                                                        ...
                                                        <div className="w-full aspect-square">
                                                            ...
                                                            <div className="bg-[#F9F8F4] p-4 rounded-lg border border-black/5 h-full">
                                                                <h4 className="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Descripción Física & Rasgos</h4>
                                                                <textarea
                                                                    className="w-full h-32 bg-transparent text-xs font-serif italic text-gray-600 outline-none resize-none placeholder:text-gray-300"

        ...
        <div className="h-full w-full flex flex-col bg-white md:bg-transparent overflow-y-auto scrollbar-hide">
            ...
            <div className="shrink-0 text-center space-y-2 max-w-4xl mx-auto pt-2 pb-1 px-4 z-10 relative">
                <div className="space-y-0.5">
                    <h1 className="text-sm md:text-xl font-sans font-thin text-zinc-900 leading-tight tracking-wide uppercase">
                        Tu carrera actoral merece <br /> <span className="font-normal text-primary">una estrategia ganadora.</span>
                    </h1>
            ...
            <div className="flex-1 relative flex flex-col justify-center w-full max-w-[100vw] overflow-hidden pb-2">
                ...
                <div
                    ...
                    className="flex overflow-x-auto snap-x snap-mandatory gap-4 px-[12vw] md:px-0 w-full items-center scrollbar-hide lg:grid lg:grid-cols-4 lg:gap-6 lg:px-0 lg:h-auto lg:max-w-7xl lg:mx-auto py-2 touch-pan-y"
                >
                        ...
                        <div className="text-center space-y-3">
                            ...
                            <div className="space-y-2 w-full">
                                <Dialog open={isGalleryOpen} onOpenChange={(open) => {
                                    // If we are closing, but the lightbox is open, DO NOT close the gallery dialog.
                                    ...
                                    <DialogContent className="max-w-4xl bg-white border-zinc-200 text-zinc-900 max-h-[90vh] overflow-y-auto">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                                            {/* Placeholder for user to drop images */}
                    ...
                    {/* CARD 2.5: ALCHEMISTERY (New Tool) */}
                    <div className="min-w-[76vw] md:min-w-0 shrink-0 snap-center bg-zinc-900 border border-purple-500/30 rounded-3xl p-5 flex flex-col h-[62vh] md:h-auto relative shadow-2xl lg:shadow-none lg:border-0 lg:rounded-none hover:bg-zinc-800 transition-colors group justify-between overflow-hidden">
                        {/* Background Effect */}
                        ...
                        {/* Header Numérico */}
                        <div className="w-full bg-purple-900/50 py-2 text-center border-b border-purple-500/20 rounded-t-2xl lg:rounded-none -mt-5 -mx-5 mb-4 backdrop-blur-sm">
                            <span className="font-black text-purple-300 uppercase tracking-widest text-xs flex justify-center items-center gap-1">
                                <Sparkles className="w-3 h-3" /> LAB . AI
                            </span>
                        </div>

                        <div className="text-center space-y-2 relative z-10">
                            <p className="text-base font-bold text-gray-400 uppercase">Descubre la</p>
                            <p className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600 animate-gradient">ENERGÍA OCULTA</p>
                            <p className="text-sm font-bold text-gray-500 uppercase">de tus personajes.</p>
                        </div>
                        ...
                        <div className="text-center space-y-3 relative z-10">
                            <div>
                                <h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-0">ALCHEMISTERY</h3>
                                <span className="text-[9px] font-bold text-purple-400 tracking-[0.2em] uppercase">Astrología de Guión</span>
                            </div>

                            <p className="text-[10px] text-gray-400 leading-tight px-4">
                                Sube tu PDF y deja que la IA analice la psicología y carta astral de tu personaje.
                            </p>

                            <Button
                                className="w-full bg-gradient-to-r from-purple-700 to-indigo-800 hover:from-purple-600 hover:to-indigo-700 text-white font-bold tracking-wider h-10 text-xs border border-white/10"
                                onClick={() => router.push('/antigravity')}
                            >
            ...
            <Dialog open={selectedIndex !== null} onOpenChange={(open) => { if (!open) { setSelectedIndex(null); setSelectedVariant(0); } }}>
                <DialogContent
                    ...
                    >
                        ...
                        <div className="flex-1 flex items-center justify-center p-4 pb-48 w-full h-full overflow-hidden" onClick={() => setSelectedIndex(null)}>
                            <img
                                ...
                                alt="Vista ampliada"
                                className="max-w-full max-h-full object-contain rounded-md shadow-2xl animate-in zoom-in-95 duration-200 pointer-events-auto"
                                onClick={(e) => e.stopPropagation()}
                                onError={(e) => {
                                    const target = e.currentTarget;
                                    // Fallback logic could go here, but complex for now.
                                    // If we are showing a variant that doesn't exist, we might want to auto-skip?
...
const GalleryGridItem = ({ num, onClick }: { num: number, onClick: () => void }) => {
    ...
    // Hardcoded list of items that have variants (based on filesystem)
    const ITEMS_WITH_VARIANTS = [1, 2, 4, 5, 6, 8, 9, 10, 11];
    const hasVariants = ITEMS_WITH_VARIANTS.includes(num);
    ...
    useEffect(() => {
        ...
        if (hasVariants) {
            ...
            }, 1200); // 1.2 seconds per swap
        }

        return () => {
            if (intervalRef.current) clearInterval(intervalRef.current);
        }
    }, [isHovering, hasVariants]);

    const getPath = (n: number, v: number) => {
        if (v === 0) return `/trabajos/${n}.jpg`;
        return `/trabajos/${n}.${v + 1}.jpg`;
    };
    ...
    return (
        ...
        >
            <img
                src={getPath(num, currentVariant)}
                alt={`Trabajo ${num}`}
                className="w-full h-full object-cover opacity-100 transition-opacity duration-300"
                onError={(e) => {
                    // Start hidden
                    if (currentVariant !== 0) {
                        // If a variant is missing, silently fail (keep current, or loop will move on)
                        // Realistically, we'd need to know if it failed to stop cycling. 
                        // For now, let's just hide the button if main image fails, or hide image if variant fails
                        // But we want to fail gracefully.
                        const target = e.currentTarget;
                        target.style.opacity = '0'; // Hide broken variant
                    } else {
                        const target = e.currentTarget;
            ...
            />
            {currentVariant >= 0 && (
                <div className={`absolute top-2 right-2 bg-black/50 backdrop-blur-md px-1.5 py-0.5 rounded text-[9px] text-white font-bold transition-opacity duration-300 ${isHovering ? 'opacity-100' : 'opacity-0'}`}>
                    {currentVariant + 1}/3
                </div>
            )}

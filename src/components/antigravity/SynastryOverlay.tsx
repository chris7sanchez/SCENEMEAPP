    ...
    return (
        <div className="w-full flex flex-col items-center">
            ...
            <div className="w-full max-w-lg mb-8 bg-[#F9F8F4] p-4 border border-black/10 rounded">
                ...
                {/* ARIES ALIGNMENT TOGGLE */}
                <div className="flex justify-center mt-4 pt-4 border-t border-black/5">
                    <button
                        onClick={() => setAlignAries(!alignAries)}
                        className={`px-4 py-2 text-[10px] uppercase font-bold tracking-widest rounded border transition-all ${alignAries
                            ? 'bg-black text-white border-black'
                            : 'bg-white text-gray-500 border-black/10 hover:border-black/30'
                            }`}
                    >
                        {alignAries ? '★ Rotación: Fijo (0° Aries)' : '☆ Rotación: Ascendente'}
                    </button>
                </div>
            </div>

            {/* The Stage - Full Size Integration */}
            <div className="relative w-full aspect-square max-w-2xl rounded-full border border-black/10 shadow-2xl overflow-hidden flex-shrink-0 bg-[#F2F0E9] ring-8 ring-[#F2F0E9]/50">

                {/* Center Point Marker for Alignment */}
                <div className="absolute top-1/2 left-1/2 w-1 h-1 bg-black rounded-full z-50 transform -translate-x-1/2 -translate-y-1/2 opacity-50"></div>
